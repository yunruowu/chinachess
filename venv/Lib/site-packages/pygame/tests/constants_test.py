...
class KmodTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        if pygame.get_sdl_version()[0] >= 2:
            cls.constants.extend(["KMOD_LGUI", "KMOD_RGUI", "KMOD_GUI"])

    def test_kmod_existence(self):
        for k in self.constants:
            self.assertTrue(
                hasattr(pygame.constants, k), "missing constant {}".format(k)
            )
...
class KeyConstantTests(unittest.TestCase):
    def test_letters(self):
        for c in range(ord("a"), ord("z") + 1):
            c = chr(c)
            self.assertTrue(
                hasattr(pygame.constants, "K_%s" % c), "missing constant: K_%s" % c
            )
