...
class GroupSingle(AbstractGroup):
    ...
    sprite = property(
        _get_sprite, _set_sprite, None, "The sprite contained in this group"
    )
    ...
    def __call__(self, left, right):
        ...
        *collide functions. Sprites must have a "rect" and an optional "radius"
        ...
        if hasattr(left, "radius"):
            ...
            leftrect = left.rect
            leftradius = (
                ratio * 0.5 * ((leftrect.width ** 2 + leftrect.height ** 2) ** 0.5)
            )
            # store the radius on the sprite for next time
            setattr(left, "radius", leftradius)
        ...
        if hasattr(right, "radius"):
            ...
            rightrect = right.rect
            rightradius = (
                ratio * 0.5 * ((rightrect.width ** 2 + rightrect.height ** 2) ** 0.5)
            )
            # store the radius on the sprite for next time
            setattr(right, "radius", rightradius)
